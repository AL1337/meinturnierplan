...
class MTRN_Matches_Renderer {
  ...
  public function render_matches_html($matches_id, $atts = array()) {
    ...
    $language = $this->resolve_language($matches_id, $atts);
    $base_url = $this->get_embed_base_url($language);

    // Build the iframe URL
    $iframe_url = $base_url . '/displayMatches.php?' . $this->build_query_string($params);
   ...
   */
  private function resolve_language($matches_id, $atts) {
    if (!empty($atts['setlang'])) {
      return sanitize_text_field($atts['setlang']);
    }

    if (!empty($matches_id)) {
      $saved_language = get_post_meta($matches_id, '_mtrn_language', true);
