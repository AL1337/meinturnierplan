    ...
    jQuery(document).ready(function($) {
      ...
      $(".mtp-color-picker").wpColorPicker({
        change: function(event, ui) {
          // Trigger the preview update handler instead of tournament ID
          $("#mtp_width").trigger("input"); // Trigger any field in the field_list to update preview
        },
        clear: function() {
          $("#mtp_width").trigger("input"); // Trigger any field in the field_list to update preview
        }
      ...
      $("select[id^='mtp_']").on("change", function() {
        // Don't trigger preview for group field as it has its own handler
        if (this.id !== 'mtp_group') {
          // Trigger the preview update handler instead of tournament ID
          $("#mtp_width").trigger("input"); // Trigger any field in the field_list to update preview
        }
      });
      ...
      function loadGroupsForTournament(tournamentId, preserveSelection, forceRefresh) {
        ...
        $.post(ajaxurl, {
          ...
          if (response.success && response.data.groups.length > 0) {
            ...
            // Trigger preview update after groups are loaded
          ...
          // Still trigger preview update even on error
          triggerPreviewUpdate();
        });
      }

      // Helper function to trigger preview update
      function triggerPreviewUpdate() {
        // Trigger the main preview update handler
        $("#mtp_width").trigger("input");
      }
      ...
      $("#<?php echo implode(', #', $field_list); ?>").on("input change", function() {
        ...
        // Get all field values and send them to server
        var data = {
        ...
        // Map form field names to expected parameter names (remove 'mtp_' prefix)
        $("input[id^='mtp_'], select[id^='mtp_']").each(function() {
          var name = $(this).attr('name');
          if (name && name.startsWith('mtp_')) {
            var paramName = name.substring(4); // Remove 'mtp_' prefix
            if ($(this).attr('type') === 'checkbox') {
              data[paramName] = $(this).is(':checked') ? '1' : '0';
            } else {
        ...
        $("input[type='range'][id^='mtp_']").each(function() {
          var name = $(this).attr('name');
          if (name && name.startsWith('mtp_')) {
            var paramName = name.substring(4); // Remove 'mtp_' prefix
            data[paramName] = $(this).val();
